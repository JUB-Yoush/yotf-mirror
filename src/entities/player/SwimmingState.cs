...
public class SwimmingState : IPlayerState
{
    ...
    {
        player.UpdateBodyRotation(new Vector3(
            player.Camera.GlobalRotation.X,
            player.Camera.GlobalRotation.Y,
            0f
        ));

        Vector3 moveDir = Vector3.Zero;
        moveDir -= player.Camera.GlobalTransform.Basis.Z * (Input.GetActionStrength("up") - Input.GetActionStrength("down"));
        moveDir += player.Camera.GlobalTransform.Basis.X * (Input.GetActionStrength("right") - Input.GetActionStrength("left"));
